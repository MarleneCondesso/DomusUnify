    ...
    <div className="min-h-screen bg-offwhite w-full">
      <header className="sticky top-0 z-20 backdrop-blur">
        <div className="flex items-center justify-between px-4 py-3">
          ...
          <div className="text-lg font-bold text-forest">{t('settings.darkMode')}</div>
      ...
      <main className="mx-auto w-full max-w-3xl px-4 pb-16 pt-6">
        <section
          className={`rounded-3xl overflow-hidden shadow-sm ${
            isDark ? 'bg-linear-to-b from-[#22124b] to-[#0b0f0d]' : 'bg-linear-to-b from-sage-light to-offwhite'
          }`}
        >
          <div className="px-6 py-10 flex flex-col items-center">
            <SunMoon isDark={isDark} />
            <div className="mt-6 text-white/90 text-sm font-semibold">
              {isSystem ? t('darkMode.status.system') : isDark ? t('darkMode.status.dark') : t('darkMode.status.light')}
            </div>
          </div>
          ...
          <div className={`px-6 py-6 ${isDark ? 'bg-black/35' : 'bg-white/80'}`}>
            <div className="space-y-5">
              <Row
                title={t('darkMode.row.dark.title')}
                subtitle={t('darkMode.row.dark.subtitle')}
                disabled={isSystem}
              ...
              <Row
                title={t('darkMode.row.system.title')}
                subtitle={t('darkMode.row.system.subtitle')}
                checked={isSystem}
    ...
    <div className="relative h-40 w-40">
      ...
      >
        <div className="relative">
          <i className="ri-sun-fill text-[96px] leading-none text-sage-dark drop-shadow-[0_10px_35px_rgba(250,204,21,0.45)]" />
          <div className="absolute inset-0 -z-10 blur-2xl opacity-60 bg-sage-dark rounded-full" />
        </div>
      </div>
